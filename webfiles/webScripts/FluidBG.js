...
void main() {
  ...
  gl_Position = vec4(aPosition, 0.0, 1.0);
    ...
    // - gold -> fluidColor
    //
    // EDIT #1: precision fix for mobile GPUs (use highp when available)
...
void main() {
  ...
  fragColor = vec4(col, 1.0);
    ...
    // EDIT #2: DPR-aware resize with a cap + explicit CSS pixel sizing
    function resize() {
        const dpr = Math.min(window.devicePixelRatio || 1, 2); // cap at 2 for perf/thermals
        ...
        // Only reallocate if needed (avoid thrash)
    ...
    function render(now) {
        ...
        if (dt >= frameTime) {
            ...
            // Keep time bounded to avoid precision degradation on some mobile GPUs
            const t = (now * 0.001) % 1000.0;
